...
export function migrate(data: Project): Project {
  ...
  const normalizeCheck = (check: any) => {
    if (!check) return;

    if (check.type === ListNextCheck.legacyId) {
      check.type = ListNextCheck.id;
    }

    if (
      check.type === PlaceholderCheck.id ||
      check.type === PlaceholderCheck.numberId
    ) {
      const compType =
        check.comparisonType !== undefined
          ? check.comparisonType
          : check.compType;
      const parsedComp =
        compType === undefined || compType === null
          ? NaN
          : Number.parseInt(compType, 10);
      const normalizedCompType = Number.isFinite(parsedComp)
        ? parsedComp
        : ComparisonType.STRING;
      check.compType = normalizedCompType;

      if (normalizedCompType === ComparisonType.STRING) {
        check.type = PlaceholderCheck.id;
        delete check.comparisonType;
      } else {
        check.type = PlaceholderCheck.numberId;
        check.comparisonType = normalizedCompType;
      }
    }
  };
  ...
  const normalizeAction = (action: any) => {
    if (!action) return;
    const legacyViewNextActionId = "List next";
    if (action.id === legacyViewNextActionId) {
      action.id = "View Next";
    }
    if (action.check) normalizeCheck(action.check);
  };
