    ...
    <div class="moreMenu absoluteMenu" ref="moreMenu">
      <template v-if="optElem">
        ...
        >
          ...
          {{
            invisibleIDs.indexOf(optElem.id) != -1
              ? t("componentList.show", "Show")
              : t("componentList.hide", "Hide")
          }}
        </div>
      </template>
    </div>
      ...
      <div v-for="(elem, index) in components" :key="index">
        ...
        >
          <div class="itemName row">
            ...
            >
            <span class="material-icons"> {{ elem.icon }} </span
            >{{
              t(`component.${elem.name}`, t(`action.${elem.name}`, elem.name))
            }}
...
export default defineComponent({
  ...
  methods: {
    ...
    checkClose(ev: MouseEvent) {
      const menu = this.$refs.moreMenu as HTMLElement | undefined;
      if (!menu) return;
...
.item {
  ...
  &.not-hovered {
    border-left: 2px solid color.adjust($blue, $alpha: -0.5);
    // margin-left: -3px;
  }
  ...
  &.hovered {
    border-left: 2px solid color.adjust($blue, $alpha: -0.8);
    // margin-left: -3px;
  }
...
.active {
  background-color: rgba(76, 195, 255, 0.08);
